...
title GastroCare AI - REPAIR AND RECOVERY
...
echo.
echo  ================================================================
echo   GastroCare AI  -  REPAIR AND RECOVERY TOOL
echo  ================================================================
echo.
...
echo   [FAIL]  Python not found. Connect and try again.
:PY_R
echo.
...
goto :DONE
:PORT
echo. & echo  Fixing port 5075...
netstat -ano 2>nul | findstr ":5075 " >nul 2>&1
if %errorlevel%==0 (for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5075 "') do (echo   Stopping PID %%P... & taskkill /PID %%P /F >nul 2>&1)) & echo   [ OK ]  Port 5075 freed.
goto :DONE
:ALL
echo. & echo  Running all repairs...
netstat -ano 2>nul | findstr ":5075 " >nul 2>&1
if %errorlevel%==0 (for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5075 "') do taskkill /PID %%P /F >nul 2>&1)
if exist "%VENV_DIR%" rmdir /s /q "%VENV_DIR%" >> "%LOGFILE%" 2>&1
"%PYTHON_EXE%" -m venv "%VENV_DIR%" >> "%LOGFILE%" 2>&1
set "PIP=%VENV_DIR%\Scripts\pip.exe"
"%PIP%" install --upgrade pip --quiet >> "%LOGFILE%" 2>&1
...
echo  ================================================================
echo.
echo   S = Start   D = Diagnostic   X = Exit
echo.
set /p "POST=  Choice [S/D/X]: "
if /i "%POST%"=="S" call "%BASEDIR%\START_GastroCare_AI.bat"
